...
int main() {
  ...
  vector<int> Y(n); rep(i, n) cin >> Y[i];
  ...
  rep(k, 4) {
    vector<edge> E;
    conn.clear();
    rep(i, m) E.push_back({A[i], B[i], Z[i]});
    if(k&1) rep(i, n) E.push_back({i, n, X[i]});
    if((k>>1)&1) rep(i, n) E.push_back({i, n+(k&1), Y[i]});
    ll ret = kruskal(n+(k&1)+((k>>1)&1), E);
    if(conn.size() == n+(k&1)+((k>>1)&1)-1) {
      chmin(ans, ret);
    }
  }
  cout << ans << endk;
