...
void solve() {
  ...
  vector<int> ans(n);
  auto dfs = [&](auto dfs, int cur, int l, int r, int depth) -> void {
    // cout << cur << ' ' << l << ' ' << r << endl;
    ans[cur] = depth;
    if(l+1 == r) return;
    if(cur > l) dfs(dfs, pos[seg.query(l, cur)], l, cur, depth+1);
    if(cur < r-1) dfs(dfs, pos[seg.query(cur+1, r)], cur+1, r, depth+1);
  };
  dfs(dfs, pos[seg.query(0, n)], 0, n, 0);
  rep(i, n) cout << ans[i] << ' ';
  cout << endk;
}
int main() {
