...
int main() {
  ...
  int m = B.size();
  vector<SegmentTree<mint>> seg(2, SegmentTree<mint>(m, [](mint a, mint b) { return a + b; }, (mint)0));
  rep(i, n) {
    int j = id(A[i]);
    mint add0 = seg[1].query(j+1, m)+1;
    mint add1 = seg[0].query(0, j)+1;
    seg[0].update(j, seg[0].get(j)+add0);
    seg[1].update(j, seg[1].get(j)+add1);
  }
  cout << seg[0].query(0, m) + seg[1].query(0, m) - n * 2 << endk;
  return 0;
}
