...
int main() {
  ...
  rep(i, q) {
    ...
    if(t == 1) {
      x--;
      int tmp = seg.get(x);
      assert(mp[tmp].count(x));
      mp[tmp].erase(x);
      if(mp[tmp].empty()) mp.erase(tmp);
      seg.update(x, y);
      mp[y].insert(x);
    } else {
      x--;
      int mn = seg.query(x, y);
      auto itr = mp[mn].lower_bound(x);
      vector<int> ans;
      while(itr != mp[mn].end() && *itr < y) {
        ans.push_back(*itr);
        itr++;
      }
      cout << ans.size() << ' ';
    ...
    // cout << mp << endk;
    // rep(i, n) cout << seg.query(i, i+1) << ' '; cout << endk;
  }
  return 0;
}
