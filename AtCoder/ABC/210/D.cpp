...
int main() {
  ...
  ll ans = longinf;
  {
    ...
    rep(i, h) rep(j, w) C[i][j] = A[i][j] - c*i - c*j;
    SegmentTree<ll> seg(w, [](ll a, ll b) { return min(a, b); }, longinf);
    rep(i, h) rep(j, w) {
      chmin(ans, A[i][j] + c*i + c*j + seg.query(0, j+1));
      seg.update(j, min(C[i][j], seg.query(j, j+1)));
    }
  }
  {
    vector<vector<ll>> C(h, vector<ll>(w));
    ...
    SegmentTree<ll> seg(w, [](ll a, ll b) { return min(a, b); }, longinf);
    rep(i, h) rrep(j, w) {
      chmin(ans, A[i][j] + c*i - c*j + seg.query(j, w));
      seg.update(j, min(C[i][j], seg.query(j, j+1)));
    }
  }
  {
