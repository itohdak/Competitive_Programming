...
int main() {
  ...
  int n, d; cin >> n >> d;
  vector<int> A(n);
  vector<deque<int>> B(500001);
  rep(i, n) {
    cin >> A[i];
    B[A[i]].push_back(i);
  }
  SegmentTree<int> seg(500001, [](int a, int b) { return min(a, b); }, n);
  rep(i, 500001) {
    if(B[i].size()) {
      seg.update(i, B[i][0]);
    }
  ...
  rep(i, n) {
    B[A[i]].pop_front();
    if(B[A[i]].size()) {
      seg.update(A[i], B[A[i]][0]);
    } else {
      seg.update(A[i], n);
    }
    int l = seg.query(max(A[i]-d, 0), A[i]+1);
    int r = seg.query(A[i], min(A[i]+d+1, 500001));
    chmax(dp[l], dp[i]+(l<n));
    chmax(dp[r], dp[i]+(r<n));
    // cout << dp << endk;
  }
