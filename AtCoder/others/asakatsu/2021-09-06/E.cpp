...
int main() {
  ...
  rep(i, m) seg.update(P[i].second, seg.get(P[i].second)+1);
  for(int i: id) {
    while(l < m && P[l].first < Q[i].first) {
      seg.update(P[l].second, seg.get(P[l].second)-1);
      l++;
    }
    ans[i] = seg.query(Q[i].first, Q[i].second+1);
  }
